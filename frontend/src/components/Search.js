...
const Search = () => {
    ...
    const sendAPI = async (image) => {
      const res = await axios.post('https://127.0.0.1:443/api/v1/users/identify',{
    ...
    return (
      <div style={{ textAlign: "center", marginTop: "50px" }}>
        <h1>Police Camera</h1>
        <video className="stream" ref={videoRef} style={{ width: "50%", margin: "20px 0",transform: "scaleX(-1)" }} />
        <br />
        ...
        {imageSrc && (
          <div style={{ marginTop: "20px" }}>
            ...
            <img className="stream" src={imageSrc} alt="Captured" style={{ width: "50%" }} />
          </div>
        )}
      </div>
    );
